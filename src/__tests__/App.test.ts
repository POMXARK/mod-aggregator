...
describe('App Integration Tests', () => {
  ...
  it('switches back to Mods page when "Моды" button is clicked', async () => {
    ...
    const modsButton = Array.from(nav.querySelectorAll('button')).find(btn =>
      btn.textContent?.includes('Моды')
    );
  ...
  it('updates active state of navigation buttons when page changes', async () => {
    ...
    await waitFor(
      () => {
        ...
        const updatedSitesButton = Array.from(nav.querySelectorAll('button')).find(btn =>
          btn.textContent?.includes('Сайты')
        );
        expect(updatedSitesButton).toHaveClass('active');

        // Mods button should no longer be active
        const updatedModsButton = Array.from(nav.querySelectorAll('button')).find(btn =>
          btn.textContent?.includes('Моды')
        );
