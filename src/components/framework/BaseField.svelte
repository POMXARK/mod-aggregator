<script lang="ts">
  ...
  let { field, value, onValueChange, nodeId, error: externalError }: Props = $props();
...
<div class="base-field" class:has-error={displayError}>
  ...
  {:else if field.type === 'multiselect'}
    ...
    >
      {#if field.options}
        {#each field.options as option}
          <option value={option.value} selected={Array.isArray(value) && value.includes(String(option.value))} disabled={option.disabled}>
            {option.label}
          </option>
        {/each}
      {/if}
