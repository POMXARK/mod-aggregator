<script lang="ts">
  // Тестовый файл для проверки правила derived-function-call
  ...
  // ❌ Еще один неправильный паттерн
  const anotherBad = $derived(someFunction());

  console.log(badResult, goodResult);
</script>

<div>
  <h1>Test derived function call detection</h1>
  <p>Bad: {badResult}</p>
  <p>Good: {goodResult}</p>
</div>
